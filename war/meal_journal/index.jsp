		...
		<div class="col s10 m7 offset-m2" id="list_meals">
			<div class="section"
				...
				<div class="row">
					...
					<button class="waves-effect waves-light btn btn-floating green right" ng-click="enableEdit(journal_meal.main.id)" ng-show="isNumericAndGreaterThanZero(journal_meal.main.quantity) && isQuantityValid()"><i class="material-icons">edit</i></button>
					...
					<div class="col s8">
						...
						<p ng-show="mode[journal_meal.main.id] == 'edit' ">
							...
							<button class="waves-effect waves-light btn  green right" ng-disabled="!isNumericAndGreaterThanZero(journal_meal.main.quantity) || !isQuantityValid()" ng-click="updateJournalMeal(journal_meal.main.id, journal_meal.main.journal_id, journal_meal.main.quantity)"><i class="material-icons">edit</i></button>
						</p>
		...
		<div class="col s10 m7 offset-m2 hidden" id="meal_option">
			<div class="section" ng-repeat="meal in meals | filter: global.search">
				<div class="row">
					...
					<button class="waves-effect waves-light btn btn-floating red right" ng-disabled="!isNumericAndGreaterThanZero(meal_quantity[meal.id]) || meal_quantity[meal.id] == '' || meal_quantity[meal.id] > 10 || meal_quantity[meal.id] < 1" ng-click="addMealToJournal(meal.id)"><i class="material-icons left">add</i></button>
					<div class="quantity-meal input-field right"> 
						<input id="quantity_{{meal.id}}" type="text" ng-model="meal_quantity[meal.id]"  class="validate" ng-class="(meal_quantity[meal.id] == '') ? '' : (!isNumericAndGreaterThanZero(meal_quantity[meal.id]) || meal_quantity[meal.id] > 10 || meal_quantity[meal.id] < 1)  ? 'invalid' : 'valid' ">
